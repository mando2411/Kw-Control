<?php if(auth()->user()){ ?>
    ...
    <script>
        (function () {
            var bell = document.getElementById('notif-menu-dropdown-mobile');
            var panel = document.getElementById('notif-menu-panel-mobile');

            if (!bell || !panel || bell.dataset.bound === '1') {
                return;
            }
            ...
            var lastTapAt = 0;
            var onBellTap = function (event) {
                var nowTs = Date.now();
                if (nowTs - lastTapAt < 260) {
                    if (event) {
                ...
                if (typeof window.toggleDashboardNotifMenuMobile === 'function') {
            ...
            if (window.PointerEvent) {
                bell.addEventListener('pointerup', onBellTap);
            } else {
                bell.addEventListener('click', onBellTap);
            }

            document.addEventListener('click', function (event) {
                var inside = event.target.closest('#notif-menu-wrapper-mobile');
